...
def show_results():
    ...
    pygame.mixer.init(frequency=44100)
    pygame.mixer.music.load("throw_angry.mp3")
    # sound_queue = queue.Queue(maxsize=1)
    # sound_thread = threading.Thread(target=sound_loop,args=(sound_queue,))
    # sound_thread.start()
    ...
    while True:
        ...
        if len(result_queue) != 0:
            ...
            for i, result in enumerate(results):
                ...
                if text[:8] == "Throwing":
                    if sound_fps_count > 100:
                        pygame.mixer.music.play(1)
                        # sound_queue.put({"do_sound":True})
                        sound_fps_count = 0
        ...
        sound_fps_count = sound_fps_count +1
...
def main():
    ...
    try:
        ...
        pw = threading.Thread(target=show_results, args=(), daemon=True)
        pr = threading.Thread(target=inference, args=(), daemon=True)
        pw.start()
        pr.start()
        pw.join()
